...
public class TowerOfHanoi {
    ...
    public static void hanoiMove(int piece_number, char from, char to, char helper) {
        ...
        if (piece_number < 0) { return; }

        //Punkt 1: Flytt alt fra stikke C over til stikke B // Bruk stikke A som hjelpestikke
        hanoiMove(piece_number-1, from, helper, to);

        //Punkt 2: Flytt øverste brikke fra A til C // Trenger ingen hjelpestikke => flytter bare en brikke
        System.out.println("Flytter " + piece_number + " " + from + "-" + to);

        //Punkt 3: Flytt alt på stikke B (fra punkt 1) over på stikke C // Bruk stikke A som hjelpestikke
